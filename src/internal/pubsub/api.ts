...
export function publishJSON<T>(ch: ConfirmChannel, exchange: string, routingKey: string, value: T): Promise<void> {
	...
	return new Promise((resolve, reject) => {
		ch.publish(exchange, routingKey, buff, { contentType: "application/json" }, (err) => {
			if (err !== null) {
...
export async function declareAndBind(
	...
	return new Promise((resolve, reject) => {
		...
		const options = queueType === 'durable' ? { durable: true, arguments: {} } : queueType === 'transient' ? { durable: false, autoDelete: true, exclusive: true, arguments: {} } : null;
		console.log(options)
